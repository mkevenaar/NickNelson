...
function validateEnvConfigState() {
    if (!envConfigured) throw new Error("Please configure the environment first using 'shared:configureEnv()'. ");
...
export async function findCommandFiles() {
    ...
    for (const folder of folders) {
        const commandFiles = readdirSync(`src/${commandsFolder}/${folder}/`)
